...
<!-- === Pacman Mobile Build: 8-bit UI + Online Highscores + Mobile Controls + START === -->
<style>
  @import url('https://fonts.googleapis.com/css2?family=Press+Start+2P&display=swap');
  :root { --ui-gap: 16px; --pm-yellow:#ffd400; --pm-dark:#0b0b0b; --pm-grey:#1a1a1a; }
  .ui-btn{position:fixed;z-index:20;font:600 14px/1.2 'Press Start 2P',monospace;padding:.6rem .9rem;border:none;border-radius:0;box-shadow:0 4px 0 #000, inset 0 -4px 0 #c5a800, inset 0 4px 0 #ffe768;background:var(--pm-yellow);color:#000;cursor:pointer;image-rendering:pixelated}
  #btnScores{right:var(--ui-gap);top:var(--ui-gap)}
  #btnChangeName{right:var(--ui-gap);top:calc(var(--ui-gap) + 52px)}
  #nameBadge{left:var(--ui-gap);top:var(--ui-gap);right:auto}
  #scoreModal{position:fixed;inset:0;display:none;place-items:center;background:rgba(0,0,0,.55);z-index:9999}
  #scoreCard{width:min(92vw,520px);max-height:80vh;overflow:auto;background:#111;color:#fff;border-radius:0;border:4px solid #000;box-shadow:0 0 0 4px #fff inset;padding:18px;font-family:'Press Start 2P',monospace}
  #scoreCard h3{margin:.2rem 0 1rem;color:#fff;text-shadow:2px 2px 0 #000}
  #scoresList{list-style:none;margin:0;padding:0}
  #scoresList li{display:flex;justify-content:space-between;gap:12px;padding:8px 10px;border-bottom:1px solid rgba(255,255,255,.10);font-family:'Press Start 2P',monospace;font-size:12px;color:#fff}

  /* Name modal */
  #nameModal{position:fixed;inset:0;display:grid;place-items:center;background:rgba(0,0,0,.65);z-index:10000}
  #nameCard{background:#111;color:#fff;border:4px solid #000;box-shadow:0 0 0 4px #fff inset;border-radius:0;padding:18px;width:min(92vw, 420px);font-family:'Press Start 2P',monospace}
  #playerNameInput{width:100%;padding:10px;border:4px solid #000;border-radius:0;font-family:'Press Start 2P',monospace;font-size:14px;background:#ffd400;color:#000;box-shadow:inset 0 -4px 0 #c5a800,inset 0 4px 0 #ffe768;}

  /* START overlay */
  #startOverlay{position:fixed; inset:0; display:none; place-items:center; z-index:10001; background:rgba(0,0,0,.55);}
  #startCard{background:#111;color:#fff;border:4px solid #000;box-shadow:0 0 0 4px #fff inset;border-radius:0;padding:18px;width:min(92vw, 420px);text-align:center;font-family:'Press Start 2P',monospace}

  /* D‑pad */
  #dpad{position:fixed;left:0;top:0;width:140px;height:140px;opacity:.95;z-index:9998;touch-action:none;filter: drop-shadow(0 6px 0 #000);}
  #dpad button{position:absolute;width:48px;height:48px;border-radius:0;border:4px solid #000;background:#ffd400;color:#000;box-shadow:0 4px 0 #000, inset 0 -4px 0 #c5a800, inset 0 4px 0 #ffe768;font-family:'Press Start 2P',monospace}
...
<!-- top UI -->
<div id="nameBadge" class="ui-btn">👤 NAME: —</div>
<button id="btnScores" class="ui-btn">🏆 High Scores</button>
<button id="btnChangeName" class="ui-btn">👤 Name</button>
...
<div id="scoreModal"><div id="scoreCard">
  ...
  <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:10px">
    <button class="ui-btn" style="background:#ffd400;color:#000" onclick="document.getElementById('scoreModal').style.display='none'">Close</button>
  </div>
</div></div>
...
<div id="nameModal">
  <div id="nameCard">
    ...
    <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:12px">
      <button id="nameOkBtn" class="ui-btn">OK</button>
    </div>
  </div>
</div>
...
<div id="startOverlay">
  <div id="startCard">
    <div style="margin-bottom:10px">TAP TO START</div>
    <button id="btnStart" class="ui-btn">START</button>
    <div style="margin-top:10px;font-size:10px;opacity:.8">enter your name first if asked</div>
  </div>
</div>
...
<script>
/* === CONFIG for Google Sheets backend === */
...
async function loadScoresOnline(limit=20){
  const r = await fetch(`${WEB_APP_URL}?top=${limit}`); return await r.json();
}

/* === Name persistence (localStorage + cookie) === */
function setCookie(name, value, days){
  const d = new Date(); d.setTime(d.getTime() + (days*864e5));
  document.cookie = name + "=" + encodeURIComponent(value) + ";expires=" + d.toUTCString() + ";path=/";
}
function getCookie(name){
  const key=name+"="; return document.cookie.split(';').map(s=>s.trim()).find(s=>s.startsWith(key))?.slice(key.length) || "";
}
function getPlayerName(){ try{ return localStorage.getItem('pacman_name') || getCookie('pacman_name') || ""; }catch(_){ return getCookie('pacman_name') || ""; } }
function setPlayerName(v){
  const val=(v||'PLAYER').toUpperCase().slice(0,24);
  try{ localStorage.setItem('pacman_name', val); }catch(_){}
  setCookie('pacman_name', val, 365*2);
  const badge = $('#nameBadge'); if (badge) badge.textContent = "👤 NAME: " + val;
  return val;
}
...
function ensureName(){
  ...
  if (!n){ $('#nameModal').style.display='grid'; $('#playerNameInput').value='PLAYER'; setTimeout(()=>$('#playerNameInput').focus(), 50); }
  else { $('#nameModal').style.display='none'; const badge=$('#nameBadge'); if(badge) badge.textContent="👤 NAME: "+n; }
}
$('#nameOkBtn').onclick = ()=>{ setPlayerName($('#playerNameInput').value); $('#nameModal').style.display='none'; $('#canvas')?.focus(); };
$('#btnChangeName').onclick = ()=>{ $('#nameModal').style.display='grid'; $('#playerNameInput').value=getPlayerName()||'PLAYER'; setTimeout(()=>$('#playerNameInput').focus(),50); };

/* === High Scores modal === */
$('#btnScores').onclick = async ()=>{
  $('#scoreModal').style.display='grid';
  const ul = $('#scoresList'); ul.innerHTML='<li>Loading…</li>';
  try{
    const data = await loadScoresOnline(20); ul.innerHTML='';
    data.forEach((s,i)=>{ const li=document.createElement('li'); li.innerHTML=`<span>${i+1}. ${s.name}</span><strong>${s.score}</strong>`; ul.appendChild(li); });
  }catch(_){ ul.innerHTML='<li>Load failed</li>'; }
};

/* === JS bridge for autosave === */
window.pacmanSaveScore = async function(_name, score){
  const n = getPlayerName() || (_name?String(_name).toUpperCase():'PLAYER');
  await saveScoreOnline(n, Number(score)||0);
};

/* === Responsive canvas scale (keep aspect 1024x600 or 1280x720) === */
(function(){
  const canvas = document.getElementById('canvas');
  if (!canvas) return;
  canvas.style.imageRendering = 'pixelated';
  const TARGET_W = 1024, TARGET_H = 600;
  function fit(){
    ...
    Object.assign(canvas.style, { width:w+'px', height:h+'px', left:((vw-w)/2)+'px', top:((vh-h)/2)+'px', right:'unset', bottom:'unset' });
    if (window.__positionDpad) setTimeout(window.__positionDpad, 20);
  }
  window.addEventListener('resize', fit); window.addEventListener('orientationchange', fit);
  document.addEventListener('visibilitychange', fit); setTimeout(fit,50); setTimeout(fit,500);
})();

/* === D‑pad position at bottom of canvas === */
(function(){
  const dpad = document.getElementById('dpad'); const canvas = document.getElementById('canvas');
  function positionDpad(){
    if(!dpad || !canvas) return;
    const r = canvas.getBoundingClientRect();
    const padW = dpad.offsetWidth || 140;
    const padH = dpad.offsetHeight || 140;
    const margin = 12;
    const x = r.left + (r.width - padW)/2;
    const y = r.top + r.height - padH - margin;
    dpad.style.left = Math.max(8, Math.min(x, window.innerWidth - padW - 8)) + 'px';
    dpad.style.top  = Math.max(8, Math.min(y, window.innerHeight - padH - 8)) + 'px';
  }
  window.addEventListener('resize', positionDpad);
  window.addEventListener('orientationchange', positionDpad);
  window.__positionDpad = positionDpad;
  window.addEventListener('load', positionDpad);
})();

/* === START overlay (mobile) === */
(function(){
  const isMobile = ('ontouchstart' in window) || (Math.min(window.innerWidth, window.innerHeight) < 900);
  const overlay = document.getElementById('startOverlay');
  const btn = document.getElementById('btnStart');
  function showStart(){ if(isMobile) overlay.style.display='grid'; }
  function startGame(){
    overlay.style.display='none';
    ensureName();
    const canvas = document.getElementById('canvas'); canvas?.focus();
    pressKey('Enter'); pressKey('Space');
    if (window.__positionDpad) setTimeout(window.__positionDpad, 50);
  }
  btn.addEventListener('click', startGame, {passive:true});
  window.addEventListener('load', showStart, {once:true});
})();

/* === Touch handlers for D‑pad and swipe === */
(function(){
  const canvas = document.getElementById('canvas');
...
/* init name at load for badge */
window.addEventListener('load', ()=>{ const n=getPlayerName(); if(n){ const b=document.getElementById('nameBadge'); if(b) b.textContent="👤 NAME: "+n; }});
</script>
<!-- === end custom mobile build === -->
